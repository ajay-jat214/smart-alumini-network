...
function Network(props){
	...
	useEffect(()=>{
	 ...
	 let currentPost=userValues.filter(
	     (Values)=>{
	      return Values.field.toLowerCase().includes(aluminiSearch.toLowerCase());
	     }
	    )

	const paginate=(pageNumber)=>{setCurrentPage(pageNumber);console.log('pagenumber:',pageNumber)};
	...
	const search1=(event)=>{
	 ...
	 return(
	     ...
	     <InputAdornment>
	     <IconButton>
	     <SearchIcon onClick={()=>{props.search(aluminiSearch);props.search2(aluminiSearch)}}/>
	     </IconButton>
	     </InputAdornment>
	     </div>
	     </div>
