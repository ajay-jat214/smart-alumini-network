...
class Messaging extends React.Component{
  ...
  componentDidMount(){
	...
	call=()=>{
     fetch('https://smart-network.herokuapp.com/messaging', {
            ...
            .catch(err=>{console.log(err);this.setState({messages:[]});
   })
	}
 ...
 render(){
  ...
  if(this.props.array.length&&this.state.messages.length!==this.props.array.length){
          ...
          <div className="scrollll ">
            {
              ...
              ?
             	  this.props.array.map((message,i)=>{return <MessagePlacing value={this.props.array[i]}  />})
              :
                null
            }
          </div>
